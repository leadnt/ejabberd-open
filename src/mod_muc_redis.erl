...
register_room(Host, Room, Pid) ->
    ...
    redis_link:qp(?MUC_REDIS_TABLE,[["HSET", Room, Host, term_to_binary(M)],["HSET",muc_to_key(Host),Room,term_to_binary(M)]]).



get_muc_room_pid(Room,Host) ->
    case catch redis_link:hash_get(?MUC_REDIS_TABLE,Room,Host) of
    {ok,undefined} ->
        [];
    {ok,M}  ->
        [binary_to_term(M)];
...
%    M = #muc_online_room{name_host = {Room, Host},   pid = Pid},
    redis_link:qp(?MUC_REDIS_TABLE,[["HDEL", Room, Host],["HDEL",muc_to_key(Host),Room]]).
...
get_vh_rooms(Host) ->
    case catch redis_link:redis_cmd(?MUC_REDIS_TABLE,["HVALS", muc_to_key(Host)]) of
    {ok,L} when is_list(L) ->
        lists:map(fun(I) ->
...
get_all_rooms(Host) ->
    case catch redis_link:redis_cmd(?MUC_REDIS_TABLE,["KEYS",<<"*">>]) of
    {ok,L} when is_list(L) ->
        lists:flatmap(fun(I) ->
                case str:str(I,<<"get_online_flags">>) of
                0 ->
                    case catch redis_link:redis_cmd(?MUC_REDIS_TABLE,[<<"hget">>,I,Host]) of
...
test1(_Host, Domain) ->
    {ok,L} =  redis_link:redis_cmd(?MUC_REDIS_TABLE,["KEYS",<<"*">>]) ,
     lists:foreach(fun(I) ->
                case str:str(I,<<"get_online_flags">>) of
                0 ->
                    case catch redis_link:redis_cmd(?MUC_REDIS_TABLE,[<<"hget">>,I,Domain]) of
                    {ok,M} when is_binary(M) ->
                        redis_link:redis_cmd(?MUC_REDIS_TABLE,["DEL",I]);
                    _ ->
                        ok
                    end;
                 _ ->
