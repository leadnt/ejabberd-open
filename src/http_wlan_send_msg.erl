...
http_send_message(Json) ->
	...
	Res = http_send_message(Server,Json),
    ?DEBUG("res ~p ,Json ~p ~n",[Res,Json]),
    Res.
...
send_chat_msg(_Server,_From,undefined,_Host,_Body,_Msg_Type,_Extend_Info) ->
	http_utils:gen_result(false, 1,<<"To no suit">>,<<"">>);
send_chat_msg(_Server,_From,_To,_Host,undefined,_Msg_Type,_Extend_Info) ->
			...
			SendRes =
				lists:flatmap(fun({obj,[{"user",ToU}]}) ->
					...
					error ->
						[<<",">>,ToU];
					JTo ->
						Packet = qtalk_public:make_message_packet(<<"chat">>,Body,Extend_Info,Msg_Type),
