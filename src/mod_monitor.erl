...
login_out_time_sum(User) ->
	...
	[] ->
		User_online_status =
			#online_status{user = User,online_time = 0,last_login_in = qtalk_public:get_timestamp()},
		catch ets:insert(user_online_status,User_online_status);
    [Online_status] ->
		Now = qtalk_public:get_timestamp(),
...
user_online_day_sum(Server) ->	
	...
	Date = 
		case qtalk_public:timestamp_to_datetime(qtalk_public:get_timestamp() - 3600) of
