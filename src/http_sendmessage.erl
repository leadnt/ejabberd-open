			...
			_ ->
				lists:foreach(fun({U}) ->
						...
						error ->
							 ok;
						_ ->
							Packet = qtalk_public:make_message_packet(Type,Body,Extend_Info,Msg_Type),
...
send_muc_msg(Server,User,Room,Host,Domain,Body,Extend_Info,Msg_Type) ->
	...
	true ->
		...
		[{obj,[{"User",Muc}]}] ->
			...
			undefined ->
				rfc4627:encode({obj,[{"data",<<"Message Body is Null">>}]});
			_ ->
		%		case qtalk_public:get_user_room_rescource(str:concat(<<"conference.">>,Server),User,Server,Muc,Domain) of
		%		[] ->
...
%				case catch qtalk_public:check_user_reg_muc(Host,Muc,User) of
				true ->
