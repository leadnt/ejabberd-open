...
-export([handle_mask_user/3,handle_virtual_user/3]).
...
-record(mask_user,{user,mask_users}).
...
stop(Host) ->
    ...
    gen_iq_handler:remove_iq_handler(ejabberd_local, Host, ?NS_SET_BLOCKED),
    gen_iq_handler:remove_iq_handler(ejabberd_sm, Host, ?NS_SET_BLOCKED),

    gen_iq_handler:remove_iq_handler(ejabberd_local, Host, ?NS_CANCEL_BLOCKED),
    gen_iq_handler:remove_iq_handler(ejabberd_sm, Host, ?NS_CANCEL_BLOCKED),

    gen_iq_handler:remove_iq_handler(ejabberd_local, Host, ?NS_RECENT_CONTACT),
    gen_iq_handler:remove_iq_handler(ejabberd_sm, Host, ?NS_RECENT_CONTACT),

    gen_iq_handler:remove_iq_handler(ejabberd_local, Host, ?NS_MUC_CONTACT),
    gen_iq_handler:remove_iq_handler(ejabberd_sm, Host, ?NS_MUC_CONTACT),
    ...
    gen_iq_handler:remove_iq_handler(ejabberd_local, Host, ?NS_VIRTUAL_USER),
    gen_iq_handler:remove_iq_handler(ejabberd_sm, Host, ?NS_VIRTUAL_USER),

    gen_iq_handler:remove_iq_handler(ejabberd_local, Host, ?NS_MAC_PUSH_NOTICE),
    gen_iq_handler:remove_iq_handler(ejabberd_sm, Host, ?NS_MAC_PUSH_NOTICE).
...
start(Host, Opts) ->
    ...
    gen_iq_handler:add_iq_handler(ejabberd_sm, Host, ?NS_SET_BLOCKED, ?MODULE, set_blocked_user, IQDisc),
    gen_iq_handler:add_iq_handler(ejabberd_local, Host, ?NS_SET_BLOCKED, ?MODULE, set_blocked_user, IQDisc),

    gen_iq_handler:add_iq_handler(ejabberd_sm, Host, ?NS_CANCEL_BLOCKED, ?MODULE, cancel_blocked_user, IQDisc),
    gen_iq_handler:add_iq_handler(ejabberd_local, Host, ?NS_CANCEL_BLOCKED, ?MODULE, cancel_blocked_user, IQDisc),
    ...
    gen_iq_handler:add_iq_handler(ejabberd_sm, Host, ?NS_MUC_CONTACT, ?MODULE, get_muc_contact, IQDisc),
    gen_iq_handler:add_iq_handler(ejabberd_local, Host, ?NS_MUC_CONTACT, ?MODULE, get_muc_contact, IQDisc),

    gen_iq_handler:add_iq_handler(ejabberd_sm, Host, ?NS_MUC_CONTACT, ?MODULE, get_muc_contact, IQDisc),
    gen_iq_handler:add_iq_handler(ejabberd_local, Host, ?NS_MUC_CONTACT, ?MODULE, get_muc_contact, IQDisc),

    gen_iq_handler:add_iq_handler(ejabberd_sm, Host, ?NS_VIRTUAL_USER, ?MODULE, handle_virtual_user, IQDisc),
    gen_iq_handler:add_iq_handler(ejabberd_local, Host, ?NS_VIRTUAL_USER, ?MODULE, handle_virtual_user, IQDisc),

    gen_iq_handler:add_iq_handler(ejabberd_sm, Host, ?NS_MAC_PUSH_NOTICE, ?MODULE, mac_push_notice, IQDisc),
    gen_iq_handler:add_iq_handler(ejabberd_local, Host, ?NS_MAC_PUSH_NOTICE, ?MODULE, mac_push_notice, IQDisc).
...
mac_push_notice(From, _To, #iq{type = Type, sub_el = SubEl} = IQ) ->
    case {Type, SubEl} of
...
set_blocked_user(From, _To, #iq{type = Type, sub_el = SubEl} = IQ) ->
    case {Type,SubEl} of
...
cancel_blocked_user(From, _To, #iq{type = Type, sub_el = SubEl} = IQ) ->
    case {Type,SubEl} of
        {set,#xmlel{name = <<"block_user">>}} ->
...
get_muc_contact(From, _To, #iq{type = Type, sub_el = SubEl} = IQ) ->
    case {Type,SubEl} of
...
set_block_user(From,El) ->
    ...
    V = case fxml:get_tag_attr_s(<<"jid">>,El) of
        <<"">> -> <<"Null">>;
        J ->
            case catch ejabberd_sql:insert_user_block_list(LServer,User,J) of
                {updated,1} -> <<"sucess">>;
                {error,Reason}  ->
    ...
    #xmlel{name = <<"block_user">>,
           attrs = [{<<"xmlns">>,?NS_SET_BLOCKED}, {<<"result">>,V}],
           children = []}.
...
cancel_block_user(From,El) ->
    ...
    #xmlel{name = <<"block_user">>,
           attrs = [{<<"xmlns">>,?NS_CANCEL_BLOCKED}, {<<"result">>,V}],
           children = []}.
...
get_recent_contact_user(From) ->
    ...
    LServer = jlib:nameprep(From#jid.server),
    Concat_users = 
        case catch qtalk_sql:get_concats(LServer,User) of
            {selected, _ , SRes} when is_list(SRes) -> lists:usort(lists:concat(SRes));
            _ -> []
        end,
    Block_users = case catch ejabberd_sql:sql_query(LServer, [<<"select blockuser from user_block_list where username = '">>,User,<<"';">>]) of
        {selected, [<<"blockuser">>], SRes1} when is_list(SRes1) -> lists:concat(SRes1);
        _  -> []
    end,

    Final_users = lists:foldl(fun(U,Acc) ->
        case lists:member(U,Acc) of
            true -> lists:delete(U,Acc);
            _ -> Acc
    end end, Concat_users, Block_users),

    #xmlel{name = <<"recent_contact_user">>,
           attrs = [{<<"xmlns">>,?NS_RECENT_CONTACT}, {<<"conctat_user">>,list_to_binary(spell_user(Final_users))},{<<"block_user">>,list_to_binary(spell_user(Block_users))}],
           children = []}.

get_muc_recent_contact(From) ->
    User = From#jid.user,
    LServer = jlib:nameprep(From#jid.server),
    Rooms = case catch qtalk_sql:get_muc_concats(LServer,User) of
        {selected, _, SRes} when is_list(SRes) -> lists:usort(lists:concat(SRes));
        _ -> []
    end,

    #xmlel{name = <<"recent_contact_muc">>,
           attrs = [{<<"xmlns">>,?NS_MUC_CONTACT}, {<<"conctat_rooms">>,list_to_binary(spell_user(Rooms))}],
           children = []}.
...
get_mac_push_notice(From) ->
    ...
    case catch ets:select(mac_push_notice, [{#mac_push_notice{user = {'$1','$2'}, _ = '_'},[{'==', '$1', User}], ['$2']}]) of
        [] -> [];
        UL when is_list(UL) -> UL;
        _ -> []
    end,

    lists:map(fun(U) ->
        #xmlel{name = <<"mac_push_notice">>,
              attrs =  [{<<"xmlns">>,?NS_MAC_PUSH_NOTICE}, {<<"shield_user">>, U}],
              children = []}  end, UserList).
...
get_mac_push_notice(From,El) ->
    ...
    #xmlel{name = <<"get_mac_push_notice">>,
           attrs = [{<<"xmlns">>,?NS_MAC_PUSH_NOTICE}, {<<"result">>,V}],
           children = []}.
...
set_mac_push_notice(From,El) ->
        ...
        J ->
           case ets:lookup(mac_push_notice,{User,J}) of
               [] ->
                   ...
                   case catch ejabberd_sql:sql_query(LServer, [<<"insert into mac_push_notice(user_name,shield_user) values ('">>,User,<<"','">>,J,<<"');">>]) of
                       {updated, 1} -> <<"sucess">>;
                       _ -> <<"failed">>
                   end;
               _ -> <<"sucess">>
    ...
    #xmlel{name = <<"mac_push_notice">>,
           attrs = [{<<"xmlns">>,?NS_MAC_PUSH_NOTICE}, {<<"result">>,V},{<<"type">>,<<"set">>}],
           children = []}.
...
cancel_mac_push_notice(From,El) ->
    ...
    V = case fxml:get_tag_attr_s(<<"jid">>,El) of
        <<"">> -> <<"failed">>;
        J ->
            case catch ejabberd_sql:sql_query(LServer, [<<"delete from mac_push_notice where user_name = '">>,User,<<"' and shield_user = '">>,J,<<"';">>]) of
                {updated, 1} ->
                    case ets:lookup(mac_push_notice,{User,J}) of
                        [] -> <<"sucess">>;	
                        _->
...
handle_virtual_user(From, To, #iq{type = Type, sub_el = SubEl} = IQ) 	->
    case {Type, SubEl} of
        {get, #xmlel{name = <<"real_user_start_session">>}} -> IQ#iq{type = result, sub_el = real_user_start_session(From,SubEl)};
        {get, #xmlel{name = <<"get_virtual_user">>}} -> IQ#iq{type = result, sub_el = get_virtual_user(From)};
        {get, #xmlel{name = <<"get_virtual_user_role">>}} -> 
                    IQ#iq{type = result, sub_el = get_virtual_user_role(From)};
        {set, #xmlel{name = <<"virtual_user_end_session">>}} -> IQ#iq{type = result, sub_el = virtual_user_end_session(From,SubEl)};
        {set, #xmlel{name = <<"real_user_end_session">>}} -> IQ#iq{type = result, sub_el = real_user_end_session(From,SubEl)};
        _ -> IQ#iq{type = error,sub_el = [SubEl, ?ERR_FEATURE_NOT_IMPLEMENTED]}
    end.
...
%    UL = 
%        case catch redis_link:hash_get(LServer,3,User,<<"mask_user">>) of
%        {ok,L } when is_list(L) ->
%                L;
%        _ ->
%            []
...
set_mask_user(From,El) ->
    ...
    V = case fxml:get_tag_attr_s(<<"jid">>,El) of
        <<"">> -> <<"failed">>;
        J ->
            case catch  check_jid_invaild(J) andalso nolimit_mask_user(J) of
            true ->
                XMLNS = ?NS_MASK_USER,
                Name = <<"mask_user">>,
                Attrs = [{<<"jid">>,J}],
                send_presence_notice(From,XMLNS,Name,Attrs),
              %  mod_update_v2:update_user_mask_redis(LServer,User,J,true),
...
cancel_mask_user(From,El) ->
    ...
    V = case fxml:get_tag_attr_s(<<"jid">>,El) of
        <<"">> -> <<"failed">>;
        J ->
            case catch  check_jid_invaild(J) of
            true ->
                XMLNS = ?NS_MASK_USER,
          ...
          %      mod_update_v2:del_user_mask_redis(LServer,User,J,true),
                lists:foreach(fun(ONode) ->
...
do_real_user_start_session(From,El) ->
            ?DEBUG("Packet ~p ~n",[El]),
    case fxml:get_tag_attr_s(<<"jid">>,El) of
        false -> {<<"result">>,<<"start session failed">>};
        JID ->
            Session = case ets:lookup(virtual_session, list_to_binary(lists:sort([From#jid.luser,JID]))) of
                S when is_record(S,virtual_session) -> S;
                _ -> init_session(From#jid.luser,JID) 
            end,
            case Session of
                undefined -> [{<<"result">>,<<"start session failed">>},{<<"xmlns">>,?NS_VIRTUAL_USER}];
                _ ->
                    send_welcome_msg(From,JID,Session),
                    [{<<"result">>,<<"start session sucess">>},{<<"xmlns">>,?NS_VIRTUAL_USER},{<<"real_user">>,Session#virtual_session.ruser}]
        end
    end.


send_welcome_msg(From,JID,Session) ->
    case jlib:make_jid({JID,From#jid.lserver,<<"">>}) of
    error ->
        ok;
    To ->
        case jlib:make_jid({Session#virtual_session.ruser,From#jid.lserver,<<"">>}) of
        error ->
            ok;
        RealFrom ->
            Type = <<"consult">>,
            Msg = list_to_binary([ <<"当前值班人员："/utf8>>,Session#virtual_session.ruser,<<" 正在为您服务."/utf8>>]),
            {MegaSecs, Secs,_MicroSec} = os:timestamp(),
            Bid = uuid:to_string(uuid:uuid1()),
            Channelid = rfc4627:encode({obj, [{"d", <<"send">>}, {"cn", <<"qchat">>}, {"usrType", <<"common">>}]}),
            Packet = 
                fxml:to_xmlel(
                    {xmlel  ,<<"message">>, [{<<"type">>,Type},{<<"qchatid">>,<<"5">>},{<<"channelid">>,Channelid},
                                        {<<"from">>,jlib:jid_to_string(RealFrom)},{<<"realfrom">>,jlib:jid_to_string(RealFrom)},
                                        {<<"to">>,jlib:jid_to_string(To)},{<<"realto">>,jlib:jid_to_string(jid:remove_resource(From))}],
                    [{xmlel,<<"body">>,[{<<"id">>,Bid},{<<"msgType">>,<<"1">>},{<<"extendInfo">>,<<"">>}],[{xmlcdata, Msg}]}]}), 
            ?DEBUG("Packet ~p ~n",[Packet]),
                ejabberd_router:route(RealFrom, To,Packet)  
         end
    end.
...
init_session(From,To) ->
   ...
   UUID = iolist_to_binary([integer_to_binary(random:uniform(65536)) | [jlib:integer_to_binary(X)|| X <- tuple_to_list(os:timestamp())]]),
    case assign_virtual_user(To) of
        ...
        Assign_User ->
            Session = #virtual_session{key = list_to_binary(lists:sort([From,To])),session = UUID,
                                       ...
                                       consumer = From,stime = qtalk_public:get_timestamp()},
...
assign_virtual_user(Virtual) ->
    case catch ets:lookup(virtual_user,Virtual) of
        [] -> [];
        [{_,L}] when is_list(L) -> lists:nth(erlang:phash(os:timestamp(), length(L)), L);
        _ -> []
    end.
...
update_nodes_virtual_session(Session) ->
    ...
    lists:foreach(fun(N) ->
        ...
        case str:str(Node,<<"ejabberd">>) of
            0 -> ok;
            _ -> catch gen_server:cast({'mod_update_v2',N},{update_virtual_session,Session})
        end
    end,L).
        

virtual_user_end_session(From,El) ->
    Res = case fxml:get_tag_attr_s(<<"jid">>,El) of
        <<"">> -> <<"failed">>;
        JID -> do_virtual_user_end_session(From#jid.luser,JID)    
    end,

    [#xmlel{name = <<"end_session">>,
            attrs = [{<<"xmlns">>,?NS_VIRTUAL_USER}, {<<"result">>,Res}]}].
...
end_nodes_virtual_session(From,To) ->
    ...
    lists:foreach(fun(N) ->
        ...
        case str:str(Node,<<"ejabberd">>) of
            0 -> ok;
            _ -> catch gen_server:cast({'mod_update_v2', N}, {delete_virtual_session, From, To})
        end
    end, L).
...
real_user_end_session(From,El) ->
    Res = case fxml:get_tag_attr_s(<<"jid">>,El) of
        <<"">> -> <<"failed">>;
        JID -> do_real_user_end_session(From#jid.luser,JID)
    end,
...
get_virtual_user(From) ->
    case catch ejabberd_sql:sql_query(From#jid.lserver,[<<"select distinct(virtual_user) from virtual_user_list;">>]) of
        {selected,_,L} when is_list(L)->
            lists:map(fun(U) ->
                #xmlel{name = <<"virtual_user">>,
                        attrs = [{<<"xmlns">>,?NS_VIRTUAL_USER},{<<"virtual_user">>,U}],
                        children = []}
            end, L);
        _ ->
            []
...
get_virtual_user_role(From) ->
    case catch  ejabberd_sql:sql_query(From#jid.lserver, [<<"select virtual_user from virtual_user_list where real_user = '">>,From#jid.luser,<<"' and on_duty_flag = 1;">>]) of
        {selected,_,L} when is_list(L)->
            lists:map(fun(R) ->
                #xmlel{name = <<"on_duty_virtual_user">>,
                       attrs = [{<<"xmlns">>,?NS_VIRTUAL_USER},{<<"virtual_user">>,R}],children = []} end,L);
        _ -> []
    end.

handle_outdate_virtual_session(Server) ->
    Time = qtalk_public:get_timestamp(),
    Session_list = lists:flatmap(fun(S) ->
    ...
    end, ets:tab2list(virtual_session)),

    lists:foreach(fun(N) ->
        Node =  list_to_binary(atom_to_list(N)),
        case str:str(Node,<<"ejabberd">>) of
            0 -> ok;
            _ -> catch gen_server:cast({'mod_update_v2', N}, {delete_virtual_sessions, Session_list})
        end
    end, nodes()). 
...
nolimit_mask_user(User) ->
	true.
%    case catch ets:lookup(nlimit_mask_user, User) of
%    [] ->
%        true;
%    _ ->
%        false
