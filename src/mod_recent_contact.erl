...
init_block_user(Server) ->
    LServer = ejabberd_sql:escape(Server),
    ets:delete_all_objects(recent_contact),
    ets:delete_all_objects(block_user),
    case catch ejabberd_sql:sql_query(LServer,[<<"select username,blockuser from user_block_list">>]) of 
        {selected, [<<"username">>,<<"blockuser">>], SRes} when is_list(SRes) ->
            lists:foreach(fun([U,B]) ->
                ets:insert(block_user,#block_user{user = U, block_user = B})
            end,SRes);
        _ -> ok
    end.
...
get_recent_contact(From, _To, #iq{type = Type, sub_el = SubEl} = IQ) ->
    case {Type, SubEl} of
        {get, #xmlel{name = <<"recent_contact">>}} -> IQ#iq{type = result, sub_el = [get_recent_contact_users(From)]};
...
handle_recent_contact(Server,User) ->
    case catch ets:lookup(recent_contact,User) of
        ...
        [Ru] when is_record(Ru,recent_contact) -> #recent_contact.recent_contact_list;
        _ -> []
    end.

get_recent_contact_user(Server,User) ->
    Cuser = case catch qtalk_sql:get_concats(Server,User) of
        {selected, [<<"u">>], SRes} when is_list(SRes) -> lists:usort(lists:concat(SRes));
         _ -> []
    end,

    Buser = case catch ets:select(block_user,[{#block_user{user = User,block_user = '$1', _ = '_'},[], ['$1']}]) of
        L when is_list(L) -> L;
        _ -> []
    end,

    Fuser = lists:foldl(fun(U,Acc) ->
...
get_block_user(From,_El) ->
    ...
    #xmlel{name = <<"block_user">>,
           attrs = [{<<"xmlns">>,?NS_BLOCK}, {<<"users">>,list_to_binary(spell_user(Buser))}],
           children = []}.
...
set_block_user(From,El) ->
    ...
    Res = case catch fxml:get_tag_attr_s(<<"jid">>,El) of
        <<"">> -> <<"Sucess">>;
        J ->
            case catch qtalk_sql:insert_user_block_list(LServer,User,J) of
                {updated,1} ->
    ...
    #xmlel{name = <<"block_user">>,
           attrs = [{<<"xmlns">>,?NS_BLOCK}, {<<"result">>,Res}],
           children = []}.
...
cancel_block_user(From,El) ->
    ...
    Res = case catch fxml:get_tag_attr_s(<<"jid">>,El) of
        <<"">> -> <<"Sucess">>;
        J ->
            case catch ejabberd_sql:sql_query(LServer, [<<"delete from user_block_list where username = '">>,User,<<"' and blockuser = '">>,J,<<"';">>]) of
                {updated,1} ->
