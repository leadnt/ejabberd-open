...
get_potoheader_base_pb(Pb) ->
	...
	Pb_header when is_record(Pb_header,protoheader) ->
		Pb_header;
	_ ->
		false
	end.
...
decode_protomessage(Pb_Msg) ->
	...
	Proto_message when is_record(Proto_message,protomessage) ->
		Proto_message;
	_ ->
		false
	end.
...
handle_header_attrs(Header) ->
    ...
    'none' ->
        [{list_to_binary(Header#stringheader.key), list_and_character_to_binary(Header#stringheader.value)}];
    V ->
        [{V,list_and_character_to_binary(Header#stringheader.value)}]
    end.
...
make_attrs_xmlels(Bodys) ->
	lists:flatmap(fun(Body) ->
			...
			true ->
				[make_attrs_xmlel(list_to_binary(Body#messagebody.value),Body#messagebody.headers,<<"">>,[])];
			_ ->
				[]
			end end,Bodys).
...
make_cdata_xmlels(Headers) ->
	lists:flatmap(fun(Header) ->
			make_cdata_xmlel(Header) end,Headers).
...
get_iqKey_type(Key) ->
    ...
    'IQKeySetUserSubScribeV2' ->
        "SET_USER_SUBSCRIBE_V2";
    'IQKeyGetUserSubScribeV2' ->
        "GET_USER_SUBSCRIBE_V2";
