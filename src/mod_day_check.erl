...
-export([find_max_memory_process/0,send_muc_service_msg/4,clear_muc_users_dimission/1]).
...
init([Server,_Opts]) ->
	Timestamp = qtalk_public:get_timestamp(),
	Tref_day = 
		case qtalk_public:get_timestamp_of_end_day(Timestamp) of
		0 ->
...
handle_info({timeout, TimerRef, first_day_sum},State=#state{server = Server}) ->
	...
	%	catch  clear_expire_muc:clear_expired_muc(Server,[{0,150,90},{150,250,75},{250,400,60},{400,1000,45}]),
		...
		catch record_redis_info(Server),
...
clear_muc_users_dimission(Server) ->
	...
	kick_dimission_mucs(LServer,100), 
	catch qtalk_sql:clear_spool(LServer),
	catch qtalk_sql:clear_user_mac_key(LServer),
	catch qtalk_sql:clear_muc_spool(LServer),
...
check_muc_last(Server) ->
	...
	lists:foreach(
			fun([Room,CreateTime]) ->
				...
				LastTime = 
					case catch qtalk_sql:get_muc_msg_last_timestamp(LServer,Room) of 
				...
				%	case mnesia:dirty_read(muc_online_room, {Room,ServerHost}) of
                    case mod_muc_redis:get_muc_room_pid(Room,ServerHost) of
					[] ->
...
judge_month_info() ->
	{{Year,Month_after,_},{_,_,_}}  =  qtalk_public:timestamp_to_datetime(qtalk_public:get_timestamp() + 24*3600*1),
	{{_,Month,_},{_,_,_}} = qtalk_public:timestamp_to_datetime(qtalk_public:get_timestamp()),
...
record_redis_info(LServer) ->
	case catch 	ejabberd_sql:sql_query(LServer,
			[<<"select user_id,user_name from host_users">>]) of
	{selected, _ , SRes} when is_list(SRes) ->
...
send_muc_service_msg(Server,Muc,Pid,Msg) ->
	...
	case catch  ejabberd_sql:sql_query(Server,
		 [<<"select show_name from muc_vcard_info where muc_name = '">>,Muc,<<"@", Domain/binary, "';">>]) of
...
destroy_muc_room(Pid,LServer,Room,ServerHost) ->
	...
	catch qtalk_sql:del_muc_users(LServer,Room,ServerHost),
	catch qtalk_sql:delete_muc_last(LServer,Room),
	catch qtalk_sql:del_user_register_mucs(LServer,Room,ServerHost),
	catch qtalk_sql:del_muc_vcard_info(LServer,Room,<<"Day check Destroy">>).

clear_redis_keys(Server,Table) ->
	case catch qtalk_sql:list_users(Server) of
	...
	_ ->
		?INFO_MSG("Now ~p get all keys Error",[qtalk_public:get_timestamp()])
	end.
...
clear_out_date_invite(Server) ->
	Time = qtalk_public:get_timestamp() - 86400*7,
...
log_destroy_room_info(Server,Room,_ServerHost) ->
        Domain = qtalk_public:get_default_domain(),
	case catch  ejabberd_sql:sql_query(Server,
		 [<<"select show_name from muc_vcard_info where muc_name = '">>,Room,<<"@", Domain/binary, "';">>]) of
	{selected,[<<"show_name">>],[[Show]]} when is_binary(Show) ->
...
kick_dimission_mucs(Server,Min_Version) ->
	case catch ejabberd_sql:sql_query(Server,
		[<<"select username,muc_name from muc_room_users where username in 
				(select user_id from host_users where version > (select max(version) - ">>,
			integer_to_binary(Min_Version),<<" from host_users ) and hire_flag = 0); ">>]) of
		...
		lists:foreach(fun([U,M]) ->
			...
			%	case mnesia:dirty_read(muc_online_room, {M,ServerHost}) of
				...
				[] ->
				    	...
				    	catch qtalk_sql:del_muc_user(Server,M,Domain,U),
		            		catch ejabberd_sql:sql_query(Server,
...
%	Flag = random:uniform(2) =:= 2,
	case catch  ejabberd_sql:sql_query(Server,[<<"select hire_flag,user_id from host_users ;">>]) of
	{selected,_ ,Res} when is_list(Res) ->
			lists:foreach(fun([Hf,U]) ->
                case catch qtalk_public:to_integer(Hf) of
				HF when HF > 0 -> 
			        case catch ejabberd_sm:get_user_resources(U,Server)  of
					[] ->
				...
				0 ->
                 ...
                 %   management_cmd:do_kick_user(U,Server);
					catch ejabberd_sm:kick_user(U,Server);
                _ ->
                    ok
				end end,Res);
		_ ->
...
clear_invalid_redis_key(Server) ->
    case catch  ejabberd_sql:sql_query(Server,[<<"select hire_flag,user_id from host_users whre hire_flag > 0;">>]) of
...
clear_user_all_redis_key(true,Server,U) ->
    ?INFO_MSG("clear all redis dismiss login key U ~p ~n",[U]),
        Host = qtalk_public:get_default_host(),
	catch redis_link:str_del(Server,5,str:concat(U,<<"@", Host/binary>>)),
...
clear_live_not_active_muc(Server) ->
    Now = qtalk_public:get_timestamp(),
        ...
        [<<"delete from muc_spool where created_at <'">>,qtalk_public:format_time(Now-3600*12),<<"';">>]),
    lists:foreach(fun(Muc) ->
       {Name,_Host} = Muc#muc_online_room.name_host,
...
%       end end,ets:tab2list(muc_online_room)). 
       end end,mod_muc_redis:get_vh_rooms(<<"conference.",Server/binary>> )). 
