...
-export([add_pid/1, remove_pid/1, get_pids/0, get_random_pid/0]).
...
get_pids() -> get_pids(?SERVER_KEY).

get_pids(Host) ->
    Rs = mnesia:dirty_read(sql_pool, Host),
    [R#sql_pool.pid || R <- Rs].

get_random_pid() -> get_random_pid(?SERVER_KEY).
...
add_pid(Pid) -> add_pid(?SERVER_KEY, Pid).
...
remove_pid(Pid) -> remove_pid(?SERVER_KEY, Pid).
