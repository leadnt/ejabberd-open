...
-export([add_muc_users/2,do_stop_muc/2,handle_ltime_not_msg_muc/2,check_invalid_Rs/1,do_kick_user/3,do_kick_user/4]).
...
get_muc_opts(Server,Args) ->
	...
	Muc_name ->
	    SName = ejabberd_sql:escape(Muc_name),
	    SHost = ejabberd_sql:escape(Conference_server),
		case catch qtalk_sql:get_muc_opts(Server,SName,SHost) of
		{selected,_, [[Opts]]} ->
...
update_muc_opts(Server,Args) ->
	...
	%		case mnesia:dirty_read(muc_online_room, {Muc_name,Conference_server}) of
            case mod_muc_redis:get_muc_room_pid(Muc_name,Conference_server) of
			[] -> 
...
%	case mnesia:dirty_read(muc_online_room, {Room,Server_Host}) of	
	...
	[M] ->
		...
		mod_muc:forget_room(LServer,Server_Host ,Room),
	    	...
	    	catch qtalk_sql:delete_muc_last(LServer,Room),
...
do_kick_user(User,Server) ->
	case ejabberd_sm_redis:get_sessions(User,Server) of
	Users when is_list(Users) ->
        [ element(2,S#session.sid) ! {kick, <<"201">>, []}  || S  <- Users],
		ok;
	_ ->
		error
	end.

do_kick_user(User,Server,Code) ->
    case ejabberd_sm_redis:get_sessions(User,Server) of
    Users when is_list(Users) ->
        [ element(2,S#session.sid) ! {kick, Code, []}  || S  <- Users],
        ok;
    _ ->
        error
    end.
...
do_kick_user(User,Server,Res,Code) ->
    case ejabberd_sm_redis:get_sessions(User,Server,Res) of
    Users when is_list(Users) ->
        [ element(2,S#session.sid) ! {kick, Code, []}  || S  <- Users],
        ok;
    _ ->
...
do_update_user_info(User,Server,Args) ->
	...
	[{Fields,Vals}] ->
        Version = 
            case catch ejabberd_sql:sql_query(Server,   [<<"select max(version) from users">>]) of
		...
		Where =  [<<"username='">>,User,<<"'">>],
		case catch ejabberd_sql:sql_transaction(Server,fun() ->
			 qtalk_sql:update_no_insert(<<"users">>,Fields ++ [<<"hire_flag">>,<<"version">>],Vals ++ [<<"1">>,Version],Where) end) of	
		{atomic,ok} ->
			ok;
		Reason ->
...
delete_user(Server,Args) ->
    ...
    undefined ->
        http_utils:gen_result(false, <<"-1">>, <<"Not find User name">>);
    User ->	
		case catch qtalk_sql:del_user(Server,User) of
		{updated,1} ->
...
restart_pgsql_odbc(Server,Args) ->
	 ...
	 Mod_R_name1 = str:concat(<<"'ejabberd_sql_sup_">>,Server),
            ...
            lists:foreach(fun({obj,[{"user",User}]}) ->
                ...
                case catch ejabberd_sql:sql_query(Server,
                    [<<"insert into muc_room_users (username,muc_name,host,login_date) values ('">>,User,<<"','">>,
                            Muc,<<"','", Host/binary, "','1459820477');">>]) of
                {updated,1} ->
                        ok;
                Res1 ->
                    ?INFO_MSG("insert muc_room_users res ~p ~n",[Res1])
                end    end, Users),
                do_stop_muc(Muc,Domain),
...
get_muc_user_name(Server) ->
    case catch ejabberd_sql:sql_query(Server,[<<"select distinct(muc_name) from muc_room_users;">>]) of
    {selected, _,M} when is_list(M) ->
        M;
    _ ->
        []
    end.

get_muc_room_name(Server,Date) ->
    case catch ejabberd_sql:sql_query(Server,[<<"select distinct(muc_room_name) from muc_room_history_backup where create_time > '">>,Date,<<"';">>]) of
    {selected, _,M} when is_list(M) ->
        M;
    _ ->
        []
...
get_muc_room_history_name(Server) ->
    case catch ejabberd_sql:sql_query(Server,[<<"select distinct(muc_room_name) from muc_room_history ;">>]) of
    {selected, _,M} when is_list(M) ->
        M;
    _ ->
        []
...
get_muc_room_history_backup_name(Server) ->
    case catch ejabberd_odbc:sql_query(Server,[<<"select distinct(muc_room_name) from muc_room_history_backup where create_time > '2017-01-01';">>]) of
    {selected, _,M} when is_list(M) ->
        M;
    _ ->
        []
...
handle_ltime_not_msg_muc(Server,Date) ->
    M1 = get_muc_user_name(Server),
    ?INFO_MSG("M1: ~p ~n",[length(M1)]),
    M2 = get_muc_room_name(Server,Date) ,
    ?INFO_MSG("M2: ~p ~n",[length(M2)]),
    M3 = get_muc_room_history_backup_name(Server),
    ?INFO_MSG("M3: ~p ~n",[length(M3)]),
    M4 = get_muc_room_history_name(Server),
    ?INFO_MSG("M4: ~p ~n",[length(M4)]),

    lists:foreach(fun([M]) ->
            case lists:member([M],M4) orelse lists:member([M],M3) of
            true ->
                [];
            _ ->
                move_muc_info(Server,M)
            end end,M1).



move_muc_info(Server,Muc) ->
    ?INFO_MSG("Move Muc ~p ~n",[Muc]),
    Domain = qtalk_public:get_default_domain(),
    ejabberd_sql:sql_query(Server,
        [<<"insert into muc_room_users_backup (muc_name , username ,host , subscribe_flag , id , date , login_date) select muc_name , username ,host , subscribe_flag , id , date , login_date from muc_room_users where muc_name = '">>,Muc,<<"';">>]),
   ejabberd_sql:sql_query(Server,
        [<<"insert into user_register_mucs_backup(username , muc_name , domain  , registed_flag) select username , muc_name , domain  , registed_flag from user_register_mucs where muc_name ='">>,Muc,<<"';">>]),
    ejabberd_sql:sql_query(Server,
        [<<"insert into muc_vcard_info_backup (muc_name,show_name,muc_desc,muc_title,muc_pic,version) select muc_name,show_name,muc_desc,muc_title,muc_pic,version from muc_vcard_info where muc_name = '">>,Muc,<<"@", Domain/binary, "'">>]),
    ejabberd_sql:sql_query(Server,
        [<<"insert into muc_room_backup(name,host,opts)  select name,host,opts from muc_room where name = '">>,Muc,<<"';">>]),

    ejabberd_sql:sql_query(Server,
        [<<"delete from muc_room_users where muc_name = '">>,Muc,<<"';">>]),
    ejabberd_sql:sql_query(Server,
        [<<"delete from user_register_mucs where muc_name = '">>,Muc,<<"';">>]),
    ejabberd_sql:sql_query(Server,
        [<<"delete from  muc_vcard_info  where muc_name = '">>,Muc,<<"@", Domain/binary, "'">>]),
    ejabberd_sql:sql_query(Server,
        [<<"delete from  muc_room where name = '">>,Muc,<<"';">>]).



check_invalid_Rs(Server) ->
        L = ejabberd_sm_redis:get_sessions(Server),
        lists:foreach(fun(S) ->
            case node(element(2, S#session.sid)) == node()  of
            true ->
                case erlang:process_info(element(2, S#session.sid)) of
                undefined ->
                    {LUser,LServer,LResource} = S#session.usr,
                    ejabberd_sm_redis:delete_session(LUser, LServer, LResource, S#session.sid),
                    qtalk_public:clear_redis_user_key(LUser, LServer, LResource) ;
                _ ->
                    ok
                end;
            _ ->
