...
get_user_friend_opts(Server,Username, Userhost, Key) ->
    get_user_friend_opts(Server, Username, Userhost).

get_user_friend_opts(Server,Username, Userhost) ->
    Sql = case ?PLATFORM of
        qchat -> [<<"select rec_msg_opt,vld_friend_opt,validate_quetion,validate_answer,version from user_relation_opts where username = '">>,Username, <<"' and userhost = '">>, Userhost, <<"';">>];
        _ -> [<<"select rec_msg_opt,vld_friend_opt,validate_quetion,validate_answer,vesion from user_relation_opts where username = '">>,Username, <<"' and userhost = '">>, Userhost, <<"';">>]
    end,
...
del_friend(Server,From,FromHost,To,Domain,Version) ->
...
add_user_friend_opts(Server,User,Userhost,Mode,Question,Answer,Version) ->
    Sql = case ?PLATFORM of
        qchat -> <<"insert into user_relation_opts(username,userhost,vld_friend_opt,validate_quetion,validate_answer,version) values ('">>;
        _ -> <<"insert into user_relation_opts(username,userhost,vld_friend_opt,validate_quetion,validate_answer,vesion) values ('">>
    end,
    case catch ejabberd_sql:sql_query(Server, [Sql,
                                                User,<<"',">>, Userhost, <<"','">>,Mode,<<",'">>,Question,<<"','">>,Answer,<<"',">>,Version,<<");">>]) of
        {updated,1} -> true;
        _ ->
            case Mode of 
                <<"2">> ->
